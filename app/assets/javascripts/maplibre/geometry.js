// isStyleLoaded() can flip back to false after style.load fires, so listen
// to both load and style.load and dispatch once from whichever fires first.
OSM.MapLibre.whenStyleReady = function (map, fn) {
...
OSM.MapLibre.addGeoJSONLayer = function (map, id, geojson, style = {}) {
  ...
  const color = style.color || OSM.MapLibre.DATA_LAYER_STYLE?.color || "#FF6200";
  const linePaint = {
    "line-color": color,
